...
\begin{align}
    w \in \mathcal{L} \Leftrightarrow \hat\Delta(S_{M_L}, w) \subseteq F_{M_L}
\end{align}
...
\begin{definition}
    ...
    \begin{itemize}
        \item $L(\emptyset) = \{\}$
        \item $\forall c \in \Sigma: L(c) \coloneqq \{c\} $
        \item $\forall r, s \in R_\Sigma: L(r \cdot s) \coloneqq L(r) L(s) = \{ u v \mid u \in L(r), v \in L(s) \}$
        \item $\forall r, s \in R_\Sigma: L(r + s) \coloneqq L(r) \cup L(s)$
        \item $\forall r \in R_\Sigma: L(r^\ast) \coloneqq L(r)^\ast = \bigcup\limits_{i=0}^{\inf} L(r)^i$
    \end{itemize}
\end{definition}
...
\begin{definition}
    ...
    the set of all words in $L(r)$ which start with $c$:

    \begin{align}
        L(D_c(r)) \coloneqq \{ w \mid c w \in L(r) \}
    \end{align}
\end{definition}
...
\begin{definition}
    ...
    \begin{align}
        ...
        \begin{cases}
            ...
            (D_c(s) \cdot t) + D_c(t)       & \text{if } r = s \cdot t \land \epsilon \in L(s).
        \end{cases}
    \end{align}
\end{definition}
...
\begin{definition}
    ...
    \begin{align}
        ...
        L(D_v(r)) &\coloneqq \{ u \mid v u \in L(r) \} \\
    \end{align}
\end{definition}
...
\begin{proof}
    We have to prove $L(D_\epsilon(r)) = L(r)$ and $\forall a \in \Sigma, w \in \Sigma^\ast: L(D_{a w}(r)) = L(D_w(D_a(r)))$.
    The former is shown by reducing $D_\epsilon(r)$.
    To show the latter, we use the definition of $L(D_w(r))$:
    \begin{align}
        L(D_w(D_a(r))) &= \{ v \mid w v \in L(D_a(r)) \}
    \end{align}

    By the definition of the derivative w.r.t. single characters,
    we know that $w v \in L(D_a(r)) \Leftrightarrow a w v \in L(r)$.
    Therefore $\{ v \mid w v \in L(D_a(r)) \} = \{ v \mid a w v \in L(r) \} = L(D_{a w}(r))$.
\end{proof}
...
\begin{theorem}
    ...
    \begin{align}
        w \in L(r) \Leftrightarrow \epsilon \in L(D_w(r))
    \end{align}
\end{theorem}

\begin{proof}
    \begin{align}
        w \in L(r) \Leftrightarrow w \epsilon \in L(r) \Leftrightarrow \epsilon \in L(D_w(r))
    \end{align}
